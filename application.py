...
def showCategory(category_name):
    ...
    items = session.query(Wallpapers).filter_by(
        category=category).order_by(asc(Wallpapers.name)).all()
    print items
...
@app.route('/wallpapers/<path:category_name>/<path:item_name>/edit',
           methods=['GET', 'POST'])
@login_check
def editWallpaper(category_name, item_name):
    ...
    # If logged in user != wallpaper owner redirect them
    if creator.id != login_session['user_id']:
        flash("You cannot edit this wallpaper. This wallpaper belongs to %s" %
              creator.name)
        return redirect(url_for('displayCategory'))
    # POST methods
    if request.method == 'POST':
...
@app.route('/wallpapers/<path:category_name>/<path:item_name>/delete',
           methods=['GET', 'POST'])
@login_check
def deleteWallpaper(category_name, item_name):
    ...
    # If logged in user != wallpaper owner redirect them
    if creator.id != login_session['user_id']:
        flash("[ERROR] Permission Denied. You cannot delete this wallpaper.")
        return redirect(url_for('displayCategory'))
    if request.method == 'POST':
...
def gconnect():
    ...
    try:
        ...
        response = make_response(
            json.dumps('[ERROR] Unable to upgrade your authorization code.'),
            401
        )
        response.headers['Content-Type'] = 'application/json'
        return response
